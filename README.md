...
3. `POST /api/compare/:exercise` downloads both from GCS, extracts frames every 0.4s (10 frames, first 4s) via ffmpeg (`server/src/video.ts` — `captureFrames`), captions each frame sequence into quantified pose descriptions via OpenAI Vision (`server/src/vision.ts` — `describeFrames`), then classifies both sets of descriptions against a fixed set of form categories (`FORM_CLASSIFICATIONS` in `server/src/pioneer.ts`, the standard `fastino/gliner2-base-v1` model — no fine-tuning) via a single call to Pioneer's GLiNER2 inference endpoint.
4. The reference video's first frame is uploaded to GCS at `reference-frames/{username}-{exercise}.jpg` and used, along with the plain-language `feedback` text, as input to fal's `veed/fabric-1.0/text` model (`server/src/veed.ts` — `generateVeedVideo`) to generate a physiotherapist-style advice video, stored at `videos/{username}-{exercise}-advice.mp4`.

Response shape: `{ feedback, referenceScores, userScores, referenceDescriptions, userDescriptions, referenceImageUrl, veedVideoUrl, veedError? }` — `referenceDescriptions`/`userDescriptions` are OpenAI Vision's raw per-frame text (useful to sanity-check what it's actually seeing); the VEED step is non-fatal — if it fails (e.g. `falKey` unset), `veedVideoUrl` is `null` and `veedError` explains why, but `feedback` is still returned.

404s tell you which of the two videos is missing.

Pioneer's real request/response shape for a `classifications` schema (confirmed live against `POST /inference`, not documented anywhere): the request needs `schema.classifications` as an array of `{ task, labels }` objects — bare label strings or `{label}` objects are both rejected. The response is `{ result: [{ data: { [task]: { label, confidence } } }, ...] }`, one **single winning label** per input text, not a score per category. `classifyDescriptions`/`compareClassifications` are built around that: reference vs. user frames are compared by how often each label wins (a distribution), not by averaging confidences. This uses Pioneer's standard `fastino/gliner2-base-v1` model — no fine-tuning job involved.
